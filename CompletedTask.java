...
public class CompletedTask implements Task {
	...
	CompletedTask(TaskId taskId, TaskInfo taskInfo) {
		// TODO JobHistoryParser.handleTaskFailedAttempt should use state from
		// the event.
	...
	private void constructTaskReport() {
		...
		report.addAllRunningAttempts(new ArrayList<TaskAttemptId>(attempts
				.keySet()));
	}
		...
		try {
			...
			for (TaskAttemptInfo attemptHistory : taskInfo.getAllTaskAttempts()
				...
				if (successfulAttempt == null
					...
					successfulAttempt = TypeConverter.toYarn(attemptHistory
							.getAttemptId());
				}
			}
