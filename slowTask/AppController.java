...
public class AppController extends Controller implements AMParams {
	...
	protected AppController(App app, Configuration conf, RequestContext ctx,
		...
		set(RM_WEB, JOINER.join(MRWebAppUtil.getYARNWebappScheme(), WebAppUtils
	...
	public void info() {
		...
		info("Application Master Overview")
				...
				._("Elasped: ",
						org.apache.hadoop.util.StringUtils.formatTime(info
								.getElapsedTime()));
	...
	public void tasks() {
		...
		if (app.getJob() != null) {
			try {
				String tt = $(TASK_TYPE);
				tt = tt.isEmpty() ? "All" : StringUtils
						.capitalize(org.apache.hadoop.util.StringUtils
								.toLowerCase(MRApps.taskType(tt).toString()));
	...
	public void attempts() {
		...
		if (app.getJob() != null) {
			try {
				...
				setTitle(join(attemptState, " ", MRApps.taskType(taskType)
						.toString(), " attempts in ", $(JOB_ID)));
	...
	public void requireJob() {
		...
		if (!checkAccess(job)) {
			accessDenied("User " + request().getRemoteUser()
					+ " does not have " + " permission to view job "
					+ $(JOB_ID));
