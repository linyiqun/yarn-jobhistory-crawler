...
import org.apache.hadoop.mapreduce.v2.hs.tool.CompletedTask;
...
/**
 * Loads the basic job level data upfront. Data from job history file is loaded
 * lazily.
 */
public class CompletedJob implements org.apache.hadoop.mapreduce.v2.app.job.Job {
	...
	public CompletedJob(Configuration conf, JobId jobId, Path historyFile,
			...
			JobACLsManager aclsMgr) throws IOException {
	...
	private void constructJobReport() {
		...
		if (getTotalReduces() == 0) {
			...
			report.setReduceProgress((float) getCompletedReduces()
					/ getTotalReduces());
		}
		...
		try {
			historyUrl = MRWebAppUtil.getApplicationWebURLOnJHSWithoutScheme(
					conf, jobId.getAppId());
	...
	public Task getTask(TaskId taskId) {
		if (tasksLoaded.get()) {
			...
			CompletedTask completedTask = new CompletedTask(taskId, jobInfo
					.getAllTasks().get(oldTaskId));
	...
	public synchronized TaskAttemptCompletionEvent[] getTaskAttemptCompletionEvents(
		...
		return getAttemptCompletionEvents(completionEvents, fromEventId,
				maxEvents);
	}
	...
	private static TaskAttemptCompletionEvent[] getAttemptCompletionEvents(
			List<TaskAttemptCompletionEvent> eventList, int startIndex,
			int maxEvents) {
		TaskAttemptCompletionEvent[] events = new TaskAttemptCompletionEvent[0];
		if (eventList.size() > startIndex) {
			int actualMax = Math
					.min(maxEvents, (eventList.size() - startIndex));
	...
	private void constructTaskAttemptCompletionEvents() {
		...
		for (Map.Entry<TaskId, Task> taskEntry : tasks.entrySet()) {
			...
			for (Map.Entry<TaskAttemptId, TaskAttempt> taskAttemptEntry : task
				...
				if (o1.getFinishTime() == 0 || o2.getFinishTime() == 0) {
					if (o1.getFinishTime() == 0 && o2.getFinishTime() == 0) {
						if (o1.getLaunchTime() == 0 || o2.getLaunchTime() == 0) {
							if (o1.getLaunchTime() == 0
									&& o2.getLaunchTime() == 0) {
								return 0;
							} else {
								long res = o1.getLaunchTime()
										- o2.getLaunchTime();
								return res > 0 ? -1 : 1;
							}
						} else {
							return (int) (o1.getLaunchTime() - o2
									.getLaunchTime());
						}
					} else {
		...
		mapCompletionEvents = new ArrayList<TaskAttemptCompletionEvent>(
				numMapAttempts);
		int eventId = 0;
		for (TaskAttempt taskAttempt : allTaskAttempts) {

			TaskAttemptCompletionEvent tace = Records
					.newRecord(TaskAttemptCompletionEvent.class);

			int attemptRunTime = -1;
			if (taskAttempt.getLaunchTime() != 0
					&& taskAttempt.getFinishTime() != 0) {
			...
			try {
				taceStatus = TaskAttemptCompletionEventStatus
						.valueOf(taStateString);
			} catch (Exception e) {
				LOG.warn("Cannot constuct TACEStatus from TaskAtemptState: ["
						+ taStateString + "] for taskAttemptId: ["
						+ taskAttempt.getID() + "]. Defaulting to KILLED");
			}
	...
	private void loadAllTasks() {
		...
		try {
			...
			for (Map.Entry<TaskID, TaskInfo> entry : jobInfo.getAllTasks()
					.entrySet()) {
	...
	protected synchronized void loadFullHistoryData(boolean loadTasks,
		...
		if (historyFileAbsolute != null) {
			...
			try {
				parser = new JobHistoryParser(
						historyFileAbsolute.getFileSystem(conf),
						historyFileAbsolute);
			...
			if (parseException != null) {
				throw new YarnRuntimeException("Could not parse history file "
						+ historyFileAbsolute, parseException);
			}
		} else {
	...
	public List<AMInfo> getAMInfos() {
		...
		for (org.apache.hadoop.mapreduce.jobhistory.JobHistoryParser.AMInfo jhAmInfo : jobInfo
				.getAMInfos()) {
			AMInfo amInfo = MRBuilderUtils.newAMInfo(
					jhAmInfo.getAppAttemptId(), jhAmInfo.getStartTime(),
					jhAmInfo.getContainerId(), jhAmInfo.getNodeManagerHost(),
					jhAmInfo.getNodeManagerPort(),
	...
	public void setQueueName(String queueName) {
		throw new UnsupportedOperationException(
				"Can't set job's queue name in history");
	}
}
